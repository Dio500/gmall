...
public class SearchServiceImpl implements SearchService {
    ...
    //ElasticsearchTemplate用的是9300 tcp端口号，但我们用的是9200 http端口号，
    //rest风格，es最新的操作对象一般都使用了RestHighLevelClient，摒弃了ElasticsearchTemplate
    @Autowired
    ...
    private SearchRequest builderQueryParam(Map<String, String> searchData) {
        //初始化
        SearchRequest searchRequest = new SearchRequest("goods_java0422");
        //条件构造器初始化
...
//            boolQueryBuilder.must(QueryBuilders.termQuery("tmName", split[1]));
        ...
        for (Map.Entry<String, String> entry : searchData.entrySet()) {
            ...
            //判断是否为平台属性，传的平台属性都加了attr_前缀（attr_操作系统=209:鸿蒙 & attr_颜色=23:红色）
        ...
        //价格查询  price=500-1000元 或 3000元以上
        ...
        //设置品牌的聚合（相当于分组）查询条件
        //terms取一个别名，field域名称（相当于group by的分组条件），size聚合多少条（不影响分页，因为默认只聚合一部分）
        //subAggregation子聚合
        builder.aggregation(
        ...
        //排序的实现 softField排序的域，softRule排序的规则     softField=price&softRule=ASC  /  DESC
    ...
    private List<SearchResponseAttrVo> getBaseAttrInfoList(Map<String, Aggregation> aggregationMap) {
        ...
        for (Terms.Bucket bucket : aggAttrIds.getBuckets()) {
            ...
            //获取平台属性值的聚合结果   红色、紫色、蓝色       高通865、麒麟890     ...
